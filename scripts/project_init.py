...
def default_skill_root(raw: str | None = None) -> Path:
    return root_resolver.resolve_skill_root(raw, script_path=__file__)
...
def create_workflow_checklist(root: Path, values: dict[str, str], args: argparse.Namespace, active_layout: layout.Layout | None = None) -> bool:
    ...
    if target.exists():
        ...
        if args.workflow_template or args.workflow_modules or args.workflow_excluded_modules:
            ...
            binding, _default_modules, _template_path = workflow.template_binding(TEMPLATE_DIR.parent, selection)
            ...
            if template_changed or module_changed:
                binding, modules, _template_path = workflow.template_binding(TEMPLATE_DIR.parent, selection)
    ...
    binding, _default_modules, template_path = workflow.template_binding(TEMPLATE_DIR.parent, selection)
    composed = composer.compose_modules(
        TEMPLATE_DIR.parent,
        root,
        selection,
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.import_task_package:
        ...
        return 0
    try:
        skill_root = default_skill_root(args.skill_root)
    except root_resolver.RootResolutionError as exc:
