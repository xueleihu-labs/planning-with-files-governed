...
def doctor(start: Path | None = None, *, planning_dir: str | Path | None = None, task_id: str | None = None) -> list[str]:
    ...
    # Detect mode: installed (site-packages) vs repository (source tree)
    # In repository mode, .sh wrappers are at staging_root/scripts/
    # _legacy/ -> _legacy -> pwf_governed -> src -> staging_root
    repo_scripts = scripts_dir.parent.parent.parent / "scripts"
    is_installed = "site-packages" in str(scripts_dir) or not repo_scripts.is_dir()
    if is_installed:
        # Installed mode (pip/pipx): wrapper scripts are not applicable
        for name in ("resolve-plan-dir.sh", "resolve-plan-dir.ps1", "inject-plan.sh", "inject-plan.ps1", "attest-plan.sh", "attest-plan.ps1"):
            lines.append(f"N/A   install surface: {name} (installed mode)")
    else:
