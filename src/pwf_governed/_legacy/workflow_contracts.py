...
def workflow_root(skill_root: Path) -> Path:
    direct = skill_root / "templates" / "workflow"
    if direct.is_dir():
        return direct
    packaged = skill_root / "resources" / "templates" / "workflow"
    if packaged.is_dir():
        return packaged
    return direct
...
def select_template(skill_root: Path, task_text: str, explicit_id: str | None = None) -> dict[str, Any]:
    from pwf_governed._legacy.workflow_template_matcher import select_template as deterministic_select
